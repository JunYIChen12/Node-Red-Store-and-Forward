...
## 文件说明
- `flows_store_forward.json`：可直接导入 Node-RED 的流程，包含 Modbus/S7 等工业协议采集、SQLite 入库、周期出队转发到 MQTT、发送后更新状态的全链路节点，以及一个“SQLite 初始化”选项卡用于部署时自动建表。
...
2. 初始化数据库：
   - **方案 A（推荐）**：导入流程后，启用“SQLite 初始化”选项卡，部署时会自动执行建表与索引创建，使用 `DB_PATH` 指向的数据库文件。
   - **方案 B**：在命令行手动运行 `sqlite3 store-and-forward.db < sqlite-init.sql`，或对自定义路径执行同名 SQL。
...
4. 部署后，流程会自动开始轮询 `pending` 数据并转发成功后标记 `sent`，断网恢复后会按顺序补发。若选择方案 A，部署时的 Inject 节点会立即完成建表，不需要手动执行 SQL。
